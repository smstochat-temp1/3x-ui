...
func ParseKeyring(active string, b64keys map[string]string) (*Keyring, error) {
	if err := validateKeyID(active); err != nil {
		return nil, fmt.Errorf("secretbox: active key id: %w", err)
	}
	if active == "" {
		return nil, errors.New("secretbox: key source has no active key id")
	}
	if len(b64keys) == 0 {
		return nil, errors.New("secretbox: key source has no keys")
	}
	kr := &Keyring{ActiveID: active, Keys: make(map[string][KeyLen]byte, len(b64keys))}
	for id, b64 := range b64keys {
		if err := validateKeyID(id); err != nil {
			return nil, fmt.Errorf("secretbox: key id %q: %w", id, err)
		}
	...
	if _, ok := kr.Keys[active]; !ok {
		return nil, fmt.Errorf("secretbox: active key %q absent from keys", active)
	}
	return kr, nil
}
...
func (f FileKeySource) Load() (*Keyring, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("secretbox: stat key file %s: %w", f.Path, err)
	}
	if runtime.GOOS != "windows" {
		if perm := info.Mode().Perm(); perm&0o077 != 0 {
			return nil, fmt.Errorf("secretbox: key file %s has insecure mode %#o (want 0600)", f.Path, perm)
		}
	}
	data, err := os.ReadFile(f.Path)
	if err != nil {
		return nil, fmt.Errorf("secretbox: read key file %s: %w", f.Path, err)
	}
	var kf keyFile
	if err := json.Unmarshal(data, &kf); err != nil {
		return nil, fmt.Errorf("secretbox: parse key file %s: %w", f.Path, err)
	}
...
func (e EnvKeySource) Load() (*Keyring, error) {
	...
	if v == "" {
		return nil, fmt.Errorf("secretbox: env %s is empty", e.Var)
	}
