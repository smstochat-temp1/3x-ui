...
│   ├── crypto/                 # At-rest node API token encryption (secretbox AES-256-GCM)
...
| **WARP / Nord / PIA** outbound integration | `service/integration/warp.go` / `nord.go` / `pia.go` | `internal/pia/` (protocol), `frontend/src/pages/xray/overrides/` |
