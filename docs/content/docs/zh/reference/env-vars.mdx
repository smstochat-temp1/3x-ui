...
写入 PIA token 或 WireGuard 私钥前，需要开启节点 token 加密（`NODE_TOKEN_ENCRYPTION=migration|required`，密钥文件权限 `0600`，或设置 `XUI_NODE_TOKEN_KEY`）。数据库备份只有使用原始加密密钥材料才能解密，无论该密钥来自密钥文件还是 `XUI_NODE_TOKEN_KEY`。参见 [出站与路由](/docs/operations/outbounds-routing)。
