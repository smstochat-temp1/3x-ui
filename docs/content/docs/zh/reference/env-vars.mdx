...
PIA 管理器始终可用。写入 PIA token 或 WireGuard 私钥前，需要开启节点 token 加密（`NODE_TOKEN_ENCRYPTION=migration|required`，密钥文件权限 `0600`，或设置 `XUI_NODE_TOKEN_KEY`）。没有密钥环的备份无法解密。参见 [出站与路由](/docs/operations/outbounds-routing)。
