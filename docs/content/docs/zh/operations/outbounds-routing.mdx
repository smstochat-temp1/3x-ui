...
对象。设置 `XUI_PIA_ENABLED=true`，并开启节点 token 加密
（`NODE_TOKEN_ENCRYPTION=migration` 或 `required`，密钥文件权限 `0600`，或
`XUI_NODE_TOKEN_KEY`）。Token 与 WireGuard 私钥只以 AES-GCM 密文保存；没有
密钥环的备份无法解密。

在 **Xray → 出站 → 更多 → PIA** 打开管理器：创建档案并登录（密码不落库）、
刷新已验签的服务器列表、按区域/服务器创建出站（不可变标签 `pia-<id>`）、
...
PIA 出站并清引用，设 `XUI_PIA_ENABLED=false`，再降级面板二进制。
