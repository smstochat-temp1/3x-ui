...
Off by default. Requires node-token encryption (`NODE_TOKEN_ENCRYPTION=migration|required` and a keyring file mode `0600`, or `XUI_NODE_TOKEN_KEY`) before a PIA token or WireGuard private key can be stored. A backup without that keyring cannot be decrypted. See [Outbounds & Routing](/docs/operations/outbounds-routing).
