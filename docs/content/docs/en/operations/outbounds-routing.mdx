...
template JSON object. Configure node-token encryption
(`NODE_TOKEN_ENCRYPTION=migration` or `required`) before
...
- A database backup can be decrypted only with the original encryption key
  material. Retain the keyring (mode `0600`) or `XUI_NODE_TOKEN_KEY` with the
  backup.
- Restore the database and the matching keyring or `XUI_NODE_TOKEN_KEY`, then
  start the panel.
