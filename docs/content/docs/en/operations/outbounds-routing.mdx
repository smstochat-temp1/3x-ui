...
template JSON object. They are available alongside NordVPN and WARP. Turn on
node-token encryption (`NODE_TOKEN_ENCRYPTION=migration` or `required`) before
storing credentials or private keys. The keyring file must be mode `0600`, or
set `XUI_NODE_TOKEN_KEY`. Tokens and WireGuard private keys are stored only as
AES-GCM ciphertext.
...
3. Choose a country, region, and WireGuard server for each outbound. Each gets
   an immutable tag `pia-<id>`.
...
- Restore: restore the database and the same keyring / `XUI_NODE_TOKEN_KEY`, then
  start the panel.
