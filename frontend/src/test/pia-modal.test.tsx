...
function PiaHarness() {
  ...
  return (
    <PiaModal
      ...
      onClose={vi.fn()}
      onAddOutbound={(outbound) => setOutbounds((prev) => [...prev, outbound])}
...
describe('PIA modal', () => {
  ...
  it('resets an existing PIA outbound in place', async () => {
    ...
    expect(payload.outbound.piaHostname).toBe('useast1');
    expect(payload.outbound.settings.secretKey).toBe('secret');
  });
});
