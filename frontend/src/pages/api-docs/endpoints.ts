...
'{\n  "success": true,\n  "obj": {\n    "items": [\n      {\n        "email": "alice@example.com",\n        "subId": "abcd1234",\n        "enable": true,\n        "totalGB": 53687091200,\n        "expiryTime": 1735689600000,\n        "limitIp": 0,\n        "limitHwid": 0,\n        "reset": 0,\n        "inboundIds": [3, 5],\n        "traffic": { "up": 1024, "down": 4096, "enable": true },\n        "createdAt": 1735000000000,\n        "updatedAt": 1735100000000\n      }\n    ],\n    "total": 2000,\n    "filtered": 47,\n    "page": 1,\n    "pageSize": 25,\n    "summary": {\n      "total": 2000,\n      "active": 1850,\n      "onlineCount": 1,\n      "depletedCount": 0,\n      "expiringCount": 0,\n      "deactiveCount": 150,\n      "online": ["alice@example.com"],\n      "depleted": [],\n      "expiring": [],\n      "deactive": ["bob@example.com"]\n    }\n  }\n}',
...

  {
    ...
    endpoints: [
      ...
      {
        method: 'GET',
        path: '/panel/api/pia/profiles',
        summary: 'List PIA account profiles (no token material).',
        responseSchema: 'PiaProfileView',
        responseSchemaArray: true,
      },
      {
        method: 'POST',
        path: '/panel/api/pia/profiles',
        summary: 'Create a PIA account profile.',
        params: [{ name: 'name', in: 'body (json)', type: 'string', desc: 'Display name.' }],
        responseSchema: 'PiaProfileView',
      },
      {
        method: 'POST',
        path: '/panel/api/pia/profiles/:uid/authenticate',
        summary: 'Exchange a PIA username and password for a token. The password is not stored.',
        params: [
          { name: 'uid', in: 'path', type: 'string', desc: 'Profile uid.' },
          { name: 'username', in: 'body (json)', type: 'string', desc: 'PIA username.' },
          { name: 'password', in: 'body (json)', type: 'string', desc: 'PIA password (write-only).' },
        ],
        responseSchema: 'PiaProfileView',
      },
      {
        method: 'PATCH',
        path: '/panel/api/pia/profiles/:uid',
        summary: 'Rename or enable/disable a PIA profile.',
        params: [
          { name: 'uid', in: 'path', type: 'string', desc: 'Profile uid.' },
          { name: 'name', in: 'body (json)', type: 'string', desc: 'Display name.', optional: true },
          { name: 'enabled', in: 'body (json)', type: 'boolean', desc: 'Whether the profile may be used.', optional: true },
        ],
        responseSchema: 'PiaProfileView',
      },
      {
        method: 'DELETE',
        path: '/panel/api/pia/profiles/:uid',
        summary: 'Delete a PIA profile and its egresses. Fails with pia_dependency_conflict when outbound tags are still referenced.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Profile uid.' }],
      },
      {
        method: 'GET',
        path: '/panel/api/pia/catalog/status',
        summary: 'Show freshness of the last signature-verified PIA server list.',
        responseSchema: 'PiaCatalogStatusView',
      },
      {
        method: 'POST',
        path: '/panel/api/pia/catalog/refresh',
        summary: 'Fetch and signature-verify the PIA server list.',
        responseSchema: 'PiaCatalogStatusView',
      },
      {
        method: 'GET',
        path: '/panel/api/pia/catalog/regions',
        summary: 'List PIA regions from the verified catalog.',
        responseSchema: 'PiaRegionView',
        responseSchemaArray: true,
      },
      {
        method: 'GET',
        path: '/panel/api/pia/catalog/regions/:id/servers',
        summary: 'List WireGuard servers in a PIA region.',
        params: [{ name: 'id', in: 'path', type: 'string', desc: 'Region id.' }],
        responseSchema: 'PiaServerView',
        responseSchemaArray: true,
      },
      {
        method: 'GET',
        path: '/panel/api/pia/egresses',
        summary: 'List managed PIA outbounds (no private keys).',
      ...
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses',
        summary: 'Create a PIA outbound with an immutable tag.',
        params: [
          { name: 'profileUid', in: 'body (json)', type: 'string', desc: 'Owning profile uid.' },
          { name: 'name', in: 'body (json)', type: 'string', desc: 'Display name.', optional: true },
          { name: 'regionId', in: 'body (json)', type: 'string', desc: 'PIA region id.' },
          { name: 'serverHostname', in: 'body (json)', type: 'string', desc: 'Pinned WireGuard server hostname.', optional: true },
          { name: 'mtu', in: 'body (json)', type: 'integer', desc: 'Tunnel MTU (default 1420).', optional: true },
          { name: 'keepaliveSeconds', in: 'body (json)', type: 'integer', desc: 'Peer keepalive (default 25).', optional: true },
          { name: 'ipv6Policy', in: 'body (json)', type: 'string', desc: 'block (default) or direct_with_warning.', optional: true },
        ],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'GET',
        path: '/panel/api/pia/egresses/:uid',
        summary: 'Get one managed PIA outbound.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' }],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'PATCH',
        path: '/panel/api/pia/egresses/:uid',
        summary: 'Update display name or tunnel options. The outbound tag is immutable.',
        params: [
          { name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' },
          { name: 'name', in: 'body (json)', type: 'string', optional: true },
          { name: 'mtu', in: 'body (json)', type: 'integer', optional: true },
          { name: 'keepaliveSeconds', in: 'body (json)', type: 'integer', optional: true },
          { name: 'ipv6Policy', in: 'body (json)', type: 'string', optional: true },
        ],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'DELETE',
        path: '/panel/api/pia/egresses/:uid',
        summary: 'Delete a PIA outbound. Requires replacementTag or deleteRules when routing still references the tag.',
        params: [
          { name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' },
          { name: 'replacementTag', in: 'body (json)', type: 'string', optional: true },
          { name: 'deleteRules', in: 'body (json)', type: 'boolean', optional: true },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses/:uid/provision',
        summary: 'Generate a WireGuard key, register it with PIA /addKey, and store the encrypted binding.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' }],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses/:uid/rotate-key',
        summary: 'Register a new key first, then switch the active binding.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' }],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses/:uid/reprovision',
        summary: 'Re-register a WireGuard key for the same pinned server.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' }],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses/:uid/test',
        summary: 'Probe the PIA outbound on the server. The private key never leaves the panel.',
        params: [
          { name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' },
          { name: 'testUrl', in: 'body (json)', type: 'string', optional: true },
          { name: 'mode', in: 'body (json)', type: 'string', optional: true, desc: 'tcp, http, or real.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses/:uid/enable',
        summary: 'Enable a ready PIA outbound so it is injected into Xray.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' }],
        responseSchema: 'PiaEgressView',
      },
      {
        method: 'POST',
        path: '/panel/api/pia/egresses/:uid/disable',
        summary: 'Disable a PIA outbound. Requires replacementTag or deleteRules when still referenced.',
        params: [
          { name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' },
          { name: 'replacementTag', in: 'body (json)', type: 'string', optional: true },
          { name: 'deleteRules', in: 'body (json)', type: 'boolean', optional: true },
      ...
      {
        method: 'GET',
        path: '/panel/api/pia/egresses/:uid/dependencies',
        summary: 'List routing, balancer, panel, node, and mtproto references to this outbound tag.',
        params: [{ name: 'uid', in: 'path', type: 'string', desc: 'Egress uid.' }],
        responseSchema: 'PiaDependencyView',
        responseSchemaArray: true,
      },
    ],
  },
